...
df2 = pd.read_csv(f'.{sep}in-game{sep}igrf.csv')
df2 = df2.sample(frac=1, random_state=918)

data = df2[['glicko', 'apm', 'pps', 'vs', 'time']]
...
data = data[['apm', 'pps', 'vs', 'gpm', 'lpm', 'apl', 'app', 'lpp', 'gpl', 'time']]
...
with open(f'.{sep}model{sep}ML_in_game.lr', 'wb') as f:
